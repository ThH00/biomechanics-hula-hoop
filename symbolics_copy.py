...
xhoop = xbar_hoop+R_hoop*(sp.cos(beta)*e1+sp.sin(beta)*e2)
    
# position vector of point on hip
# R_hip = 0.5-0.4*x3
R_hip = 0.2
xhip = xbar_hip+R_hip*sp.cos(beta)*e1+R_hip*sp.sin(beta)*e2

d2 = np.dot(xhoop-xhip,xhoop-xhip)

variables = d2.free_symbols

# d2.free_symbols
# {b, xD1, psi, xB1, xD0, xD2, xB2, phi, xB0, theta, beta, z}

# Define the array of variables that are a function of time
# (I will just take derivative with respect vars, not beta and gamma)
...
grad = [sp.diff(d2, xi) for xi in vars]
# Second derivative with respect to each variable
grad2 = [sp.diff(d2, xi, 2) for xi in vars]
...
n = sp.cos(beta)*E1+sp.sin(beta)*E2
t1 = -sp.sin(beta)*E1+sp.cos(beta)*E2
t2 = np.cross(n,t1)

minimum_distance = sp.symbols('minimum_distance')
omega_hip = sp.symbols('omega_hip:3')

# Position vectors to points P and Q
xQ = xbar_hoop+R_hoop*n
xP = xQ-minimum_distance*n
...
with open("output.txt", "a") as file:
    ...
    file.write(f'WF[0,0] = {WF2_0}\n')
    file.write(f'WF[0,1] = {WF2_1}\n')
    file.write(f'WF[0,2] = {WF2_2}\n')
    file.write(f'WF[0,3] = {WF2_3}\n')
    file.write(f'WF[0,4] = {WF2_4}\n')
    file.write(f'WF[0,5] = {WF2_5}\n\n')
               


print('The End.')

# to do, add sp to be able to add to main ConnectionAbortedError