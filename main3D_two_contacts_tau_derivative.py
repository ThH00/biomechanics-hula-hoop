...
def get_minimizing_tau(q, xbar_hip):
    ...
    minimizing_tau = np.sort(tau[min_indices]) # ordering tau in increasing order

    return minimizing_tau


def get_contact_constraints(q,u,a,xbar_hip,vbar_hip,abar_hip,tau,dtau_dt,d2tau_dt2):
...
def combine_contact_constraints(q,u,a,tau_derivatives = False):
    ...
    original_tau = get_minimizing_tau(q,xbar_hip[iter,:])

    # augment tau to guarantee a 2D array
    if np.size(original_tau) == 1:
        tau = np.array([original_tau.item(),2.1*np.pi])
    else:
        tau = original_tau

    if tau_derivatives:
        # numerical calculation of dtau_dt
        dtau_dt = np.array([0,0])
        # partial derivative of tau wrt t
        pTau_pt = (tau - minimizing_tau_save[:,iter-1])/dtime
        dtau_dt = dtau_dt+pTau_pt
        # calculating partial derivatives at t wrt xi
        vars = np.concatenate((q,xbar_hip[iter,:]))
        varsdot = np.concatenate((u,vbar_hip[iter,:]))
        epsilon = 1e-6
        dtau_dvars = np.zeros((np.size(vars),2))
        for i in range(np.size(vars)):
            epsilon_vector = np.zeros(np.size(vars))
            epsilon_vector[i] = epsilon
            incremented_combined_vector = np.concatenate((q,xbar_hip[iter,:]))+epsilon_vector
            dtau_dvars[i,:] = (get_minimizing_tau(incremented_combined_vector[0:6],incremented_combined_vector[6:9])-tau)/epsilon
            dtau_dt = dtau_dt+dtau_dvars[i,:]*varsdot[i]

        # numerical calculation of d2tau_dt2 (second derivative of tau wrt t)
        d2tau_dt2 = np.array([0,0])
    else:
        dtau_dt = np.array([0,0])
        d2tau_dt2 = np.array([0,0])
    ...
    if np.size(original_tau) == 2:  # two local minima
        gN[0], gNdot[0], gNddot[0], WN[0,:], gammaF[gammaF_lim[0,:]], gammadotF[gammaF_lim[0,:]], WF[gammaF_lim[0,:],:] = get_contact_constraints(q,u,a,xbar_hip[iter,:],vbar_hip[iter,:],abar_hip[iter,:],tau[0],dtau_dt[0],d2tau_dt2[0])
        gN[1], gNdot[1], gNddot[1], WN[1,:], gammaF[gammaF_lim[1,:]], gammadotF[gammaF_lim[1,:]], WF[gammaF_lim[1,:],:] = get_contact_constraints(q,u,a,xbar_hip[iter,:],vbar_hip[iter,:],abar_hip[iter,:],tau[1],dtau_dt[1],d2tau_dt2[1])
        # saving values
        minimizing_tau_save[:,iter] = tau 
        
    elif np.size(original_tau) == 1:
        ...
        # saving values
        minimizing_tau_save[0,iter] = original_tau.item()
...
def get_R(x, prev_x, prev_AV, prev_gammaF, prev_gdotN, prev_q, prev_u, tau_derivative, *index_sets):
    # global iter
    ...
    gN, gNdot, gNddot, WN, gammaF, gammadotF, WF = combine_contact_constraints(q,u,a,tau_derivative)
...
def get_R_J(x,prev_x,prev_AV,prev_gammaF,prev_gdot_N,prev_q,prev_u,tau_derivative):
    
    epsilon = 1e-6
    # add return of index sets here
    R_x, AV, gdot_N, gammaF, q, u, A, B, C, D, E = get_R(x,prev_x,prev_AV,prev_gammaF,
                                           prev_gdot_N,prev_q,prev_u,tau_derivative)
    ...
    for i in range(n):
        ...
        R_x_plus_epsilon,_,_,_,_,_ = get_R(x+epsilon*I[:,i],prev_x,prev_AV,
                                           prev_gammaF,prev_gdot_N,prev_q,prev_u,tau_derivative,A, B, C, D, E)
...
def update(prev_x, prev_AV, x_guess=None, tau_derivative=False):
    ...
    Res,AV_temp,gdot_N_temp,gammaF_temp,q_temp,u_temp,J\
         = get_R_J(x_temp,prev_x,prev_AV,gammaF[iter-1,:],gdotN[iter-1,:],
                   q[iter-1,:],u[iter-1,:],tau_derivative)
    ...
    while norm_R>tol_n and nu<maxiter_n:
        ...
        if nu == maxiter_n:
            ...
            if n_tau*tol_n < 1001:
                n_tau = n_tau*10
                prev_x, prev_AV = update(prev_x,prev_AV,x_temp,tau_derivative=True)
            else:
