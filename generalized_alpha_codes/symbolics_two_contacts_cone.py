...
R_hip, n = get_R_hip(dv, tau)
gN = dh - R_hip

# Right handed orthonormal basis of contact point
temp = np.cross(n,E3)
t1 = temp/(np.dot(temp,temp)**0.5)
t2 = np.cross(n,t1)
...
xP = xbar_hip+dv*E3+R_hip*n
vP = vbar_hip+np.cross(hip_ang_vel,dv*E3+R_hip*n)
...
with open("output_cone.txt", "a") as file:
    # Append the string to the file
    file.write(f'gN = {gN}\n')
    file.write(f'gNdot = {gNdot}\n')
    file.write(f'gNddot = {gNddot}\n\n')

    file.write(f'WN[0] = {WN0}\n')
    file.write(f'WN[1] = {WN1}\n')
    file.write(f'WN[2] = {WN2}\n')
    file.write(f'WN[3] = {WN3}\n')
    file.write(f'WN[4] = {WN4}\n')
    file.write(f'WN[5] = {WN5}\n\n')
