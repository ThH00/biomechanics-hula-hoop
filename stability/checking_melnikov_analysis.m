...
box on
grid on
title("A plot of $g(\alpha)$",'Interpreter','Latex')
...
for i = 1:n
    ...
    integrand_A = @(tau) 2*sqrt(alpha(i)) .* sech(sqrt(alpha(i))*tau) .* ...
        sin(2*acos(tanh(-sqrt(alpha(i))*tau)) + 2*tau + 2*t0);
    
    % Perform numerical integration
    result_A(i) = integral(integrand_A, -Inf, Inf, 'RelTol',1e-10,'AbsTol',1e-12);

    result_BC(i) = 8*sqrt(alpha(i))+2*pi;

end

beta = 1;
delta = 1;
result = beta*result_A-delta*result_BC;
